...
- `2026-08-03-silent-failure-sweep.md` — exhaustive whole-codebase hunt for silent failures (AST
  sweep of all 101 exception handlers + 4 subsystem agents + an LLM/DB external-edge review). 8 real
  findings, all fixed with regression tests: component `from_config` coercion, `null`→`"None"` ingest
  corruption, `_collect` silent drop, LLM transient-retry inconsistency, forgotten-path→`:memory:`,
  and three context-block/reference boundary gaps. Exception-handler layer and DB edge verified clean.
