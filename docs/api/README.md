...
| [`llm.md`](llm.md) | `ragkit.llm` | The model-serving/request layer: backends (schema-constrained vs. prompt-described JSON), the HTTP client and its retry/error taxonomy, the model pool (thrash guard, connection reuse), `llama-server` launch-flag rendering. |
