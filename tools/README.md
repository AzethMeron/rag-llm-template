...
| `build_vector_index.sh` | build the ANN index on a `lancedb` `[vector]` store so `search()` switches from an O(n) brute-force scan of every row to sub-linear approximate search — run once after a corpus is (mostly) embedded; a no-op on a `qdrant`-backed store (its HNSW index is built on upsert) |
| `run_recipe.sh` | run one recipe end to end against a locally-served model (serve → import → run → export → eval), the multi-command sequence every recipe README spells out by hand — cleanly stops the server afterwards (so the next recipe's models fit in VRAM) and guards against scoring a stale journal |
